...
""",
    ...
    assert health_data["app_version"] == "0.1.0"
    assert health_data["prompt_version"] == "rag-prompt-v1"
    assert health_data["embedding_version"] == "simple-hash-v1"
    assert health_data["graph_version"] == "ccar33-graph-export-v1"
    ...
    query_response = client.post(
        "/api/v1/query",
        json={"query": "What is the surge margin requirement?", "top_k": 1, "use_guardrail": True},
    )
    ...
    assert payload["promptVersion"] == "rag-prompt-v1"
    assert payload["embeddingVersion"] == "simple-hash-v1"
    assert payload["graphVersion"] == "ccar33-graph-export-v1"
