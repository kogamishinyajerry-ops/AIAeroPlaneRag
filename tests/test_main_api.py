...
""",
    ...
    assert health_data["vector_db"] in {"connected", "mock", "degraded", "failed"}
    assert health_data["llm"] in {"connected", "mock", "disconnected"}
    assert health_data["ollama"] in {"available", "unavailable", "degraded", "not_configured"}
    assert health_data["graph_db"] in {"connected", "disconnected", "mock", "degraded"}
    # ── Extended fields ───────────────────────────────────────────────────
